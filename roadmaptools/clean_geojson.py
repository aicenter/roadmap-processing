...
def create_desimplified_edge(coord_u: List[float],
                             coord_v: List[float],
                             item: Feature,
                             is_forward: bool,
                             from_id: int,
    ...
    del item['properties']['nodes']
    item['geometry']['coordinates'] = [coord_u, coord_v]

    # setting from/to nodes
    item['properties']['from_osm_id'] = from_id
    item['properties']['to_osm_id'] = to_id
    ...
    if 'oneway' not in item['properties'] or item['properties']['oneway'] != 'yes':
        ...
        elif 'lanes' in item['properties'] and _is_int(item['properties']['lanes']) and int(
                item['properties']['lanes']) >= 2:
...
def prune_geojson_file(json_dict: FeatureCollection, keep_attributes: Set[str], remove_attributes: Set[str],
    ...
    # we need to keep the list of nodes
    keep_attributes.add("nodes")
    ...
    for item in tqdm(edges, desc="Pruning geojson"):
        ...
        if desimplify:
            for j in range(0, len(item['geometry']['coordinates']) - 1):
                ...
                from_id = item['properties']['nodes'][j]
                to_id = item['properties']['nodes'][j + 1]
                new_item = create_desimplified_edge(u, v, new_edge, True, from_id, to_id)
                json_dict['features'].append(new_item)

                # create the opposite direction edge if it is not oneway
                if 'oneway' not in item['properties'] or item['properties']['oneway'] != 'yes':
                    new_edge = copy.deepcopy(item)
                    new_item = create_desimplified_edge(v, u, new_edge, False, to_id, from_id)
                    json_dict['features'].append(new_item)
        else:
